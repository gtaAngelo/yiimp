...
class YiimpUtils extends Component
{
  ...
  public function get_algo_param() {
	...
	public function getCoinPort($coin) {
		if (!$coin) return $this->getAlgoPort('scrypt');

		if (!empty($coin->dedicatedport))
			return $coin->dedicatedport;

		if ($coin->auxpow && !empty($coin->parent_coin_id)) {
			$parent = Coins::findOne($coin->parent_coin_id);
			if ($parent) {
				if (!empty($parent->dedicatedport))
					return $parent->dedicatedport;
				return $this->getAlgoPort($parent->algo);
			}
		}

		if ($coin->auxpow) {
			$parent = Coins::find()
				->where(['auxpow' => 0, 'algo' => $coin->algo, 'enable' => 1])
				->one();
			if ($parent) {
				if (!empty($parent->dedicatedport))
					return $parent->dedicatedport;
		...
		return $this->getAlgoPort($coin->algo);
	}
