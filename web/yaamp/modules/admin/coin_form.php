...
echo CUFHtml::openActiveCtrlHolder($coin, 'parent_coin_id');
echo CUFHtml::activeLabelEx($coin, 'parent_coin_id');
$parentCoins = array('' => '-- Select parent coin --');
$db_parent_coins = getdbolist('db_coins', "auxpow=0 AND enable=1 AND installed=1 ORDER BY algo, name");
foreach ($db_parent_coins as $pcoin) {
	$parentCoins[$pcoin->id] = "{$pcoin->name} ({$pcoin->symbol}) - {$pcoin->algo}";
}
echo CUFHtml::dropDownList('db_coins[parent_coin_id]', $coin->parent_coin_id, $parentCoins, array(
	'style' => 'border: 1px solid #dfdfdf; height: 30px; width: 300px',
	'class' => 'textInput tweetnews-input'
));
echo '<p class="formHint2">Parent coin for merge mining. Leave empty to auto-detect (uses first non-auxpow coin with same algo)</p>';
echo CUFHtml::closeCtrlHolder();
