...
function getCoinPort($coin) {
	if (!$coin) return getAlgoPort('scrypt');

	if (!empty($coin->dedicatedport))
		return $coin->dedicatedport;

	if ($coin->auxpow && !empty($coin->parent_coin_id)) {
		$parent = getdbo('db_coins', $coin->parent_coin_id);
		if ($parent) {
			if (!empty($parent->dedicatedport))
				return $parent->dedicatedport;
			return getAlgoPort($parent->algo);
		}
	}

	if ($coin->auxpow) {
		$parent = getdbosql('db_coins', "auxpow=0 AND algo=:algo AND enable=1", array(':algo' => $coin->algo));
		if ($parent) {
			if (!empty($parent->dedicatedport))
				return $parent->dedicatedport;
			return getAlgoPort($parent->algo);
